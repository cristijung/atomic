...
const FormContato= () => {
  ...
  const onSubmit: SubmitHandler<ContactFormData> = async (data) => {
    ...
    alert(`Mensagem enviada com sucesso por ${data.nome}! Categoria: ${data.categoriaDaMensagem}`);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 p-4 md:p-8 max-w-2xl mx-auto bg-white shadow-md rounded-lg">
      {/* campo nome */}
      <div>
      ...
      <div>
        <label htmlFor="categoriaDaMensagem" className="block text-sm font-medium text-gray-700 mb-1">
        ...
        <select
          ...
          className={`mt-1 block w-full px-3 py-2 border ${
            errors.categoriaDaMensagem ? 'border-red-500' : 'border-gray-300'
        ...
        {errors.categoriaDaMensagem && (
          <p className="mt-2 text-sm text-red-600">{errors.categoriaDaMensagem.message}</p>
        )}
      </div>     
      ...
      <div>
        ...
        />
        {errors.mensagem && <p className="mt-2 text-sm text-red-600">{errors.mensagem.message}</p>}
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-end space-y-3 sm:space-y-0 sm:space-x-3">
        <button
          ...
          className="w-full sm:w-auto inline-flex justify-center py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50"
        >
          Limpar
        </button>
        <button
